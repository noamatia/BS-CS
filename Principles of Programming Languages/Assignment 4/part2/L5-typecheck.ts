...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, Binding2, VarDecl, Binding, CExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, NonTupleTExp, isNonEmptyTupleTExp, NonEmptyTupleTExp, isNonTupleTExp } from "./TExp";
...
import { concatVars } from './L5-eval';
...
export const typeofValues = (values: ValuesExp, tenv: TEnv): Result<TExp> => {
    if(isEmpty(values.vals))
        return makeOk(makeEmptyTupleTExp());
    else{
        const tes = bind(mapResult((v) => typeofExp(v, tenv), values.vals), (t: TExp[]) => 
                        mapResult(validateNonTupleTExp, t));
        return bind(tes, (t: NonTupleTExp[]) => makeOk(makeNonEmptyTupleTExp(t)));
    }
}

export const validateNonTupleTExp = (te: TExp): Result<NonTupleTExp> =>
    isNonTupleTExp(te) ? makeOk(te):
    makeFailure ("Nested Tuple!");
...
export const concatVarsTexps = (arr:TExp[], curr: Binding2, other: Binding2[]): TExp[] =>
    isEmpty(other) ? arr.concat(map((v: VarDecl) => v.texp, curr.var)) :
    concatVarsTexps(arr.concat(map((v: VarDecl) => v.texp, curr.var)), first(other), rest(other));

export const concatA = (arr: CExp[], curr: CExp, other: CExp[]): Result<CExp[]> =>
    isValuesExp(curr) ? (
        isEmpty(other) ? makeOk(arr.concat(curr.vals)) :
        concatA(arr.concat(curr.vals), first(other), rest(other))
    ) :
    makeFailure(`let-values.vals has to be Values`);

export const w = (x: Result<CExp[]>): number =>
    isOk(x) ? x.value.length :
    -1;

export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
        const vars = concatVars([], first(exp.bindings), rest(exp.bindings));
        const valsPreFlatten = map((b: Binding2) => b.val, exp.bindings);
        const vals = concatA([], first(valsPreFlatten), rest(valsPreFlatten));
        const varTEs = concatVarsTexps([], first(exp.bindings), rest(exp.bindings));
        if(isOk(vals)){
            const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv),
                                                                (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),
                                    varTEs, vals.value);
            const varsSizes = map((b: Binding2) => b.var.length, exp.bindings);
            const valsSizes = map((x: CExp) => w(concatA([], x, [])), valsPreFlatten);
        ...
        return makeFailure(`let-values.vals has to be Values`);
}
