...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | LetValuesExp | ValuesExp;;
...
export interface Binding2 {tag: "Binding2"; var: VarDecl[]; val: CExp; }
export const makeBinding2 = (v: VarDecl[], val: CExp): Binding2 =>
    ({tag: "Binding2", var: v, val: val});
export const isBinding2 = (x: any): x is Binding2 => x.tag === "Binding2";
...
export interface LetValuesExp {tag: "LetValuesExp"; bindings: Binding2[]; body: CExp[]; }
export const makeLetValuesExp = ( bindings: Binding2[], body: CExp[]): LetValuesExp =>
...
export const parseValuesExp = (params: Sexp[]): Result<ValuesExp> =>
    safe((params: CExp[]) => makeOk(makeValuesExp(params)))
...
export const parseLetValuesExp = (bindings: Sexp,  body: Sexp[]): Result<LetValuesExp> =>
    ...
    ! isGoodBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    safe2((bdgs: Binding2[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (parseBindings2(bindings), mapResult(parseL5CExp, body));
...
export const parseBindings2 = (bindings: [Sexp, Sexp][]): Result<Binding2[]> =>
    mapResult((b: [Sexp, Sexp]) =>
    isArray(b[0]) ?
    safe2((vds: VarDecl[], vals: CExp) => makeOk(makeBinding2(vds, vals)))
    (mapResult(parseVarDecl, b[0]), parseL5CExp(b[1])):
    makeFailure('Vars of "let-values" has to be an array'),
    bindings);
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isValuesExp(e) ? safe((vals: string[]) => makeOk(`(values ${join(" ", vals)})`))
                        (mapResult(unparse, e.vals)) :
    isLetValuesExp(e) ? unparseLetValuesExp(e) :
    bind(mapResult(unparse, e.exps), (exps: string[]) => makeOk(`(L5 ${exps})`));

//******************************************************************************************************************

export const safe = <T1, T2>(f: (x: T1) => Result<T2>): (xr: Result<T1>) => Result<T2> =>
    (xr: Result<T1>) =>
        bind(xr, (x: T1) => f(x));
...
export const unparseLetValuesExp = (le: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseBindings2(le.bindings), unparseLExps(le.body));
    
export const unparseBindings2 = (bindings: Binding2[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`((${vd}) ${val})`))(unparseVarDecls(bdg.var), unparse(bdg.val)), bindings),
             (bdgs: string[]) => makeOk(join(" ", bdgs)));

export const unparseVarDecls = (vars: VarDecl[]): Result<string> => 
    bind(mapResult(unparseVarDecl ,vars),  ((vars: string[]) => makeOk(join(" ", vars))));
