...
;; <proc-te>      ::= [ <tuple-te> -> <tuple-te> ] // proc-te(param-tes: list(te), return-te: te)
...
import { safe, unparse } from "./L5-ast";
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    isArray(texp) &&  texp.indexOf('->') === -1 ? parseTupleTExp2(texp) :   
    f(texp);

export const f = (texp: Sexp): Result<NonTupleTExp> =>
    ...
    makeFailure(`Unexpected TExp2 - ${texp}`);
/*
;; expected structure: (<params> -> <returnte>)
;; expected exactly one -> in the list
;; We do not accept (a -> b -> c) - must parenthesize
*/
const parseCompoundTExp = (texps: Sexp[]): Result<NonTupleTExp> => {
    const pos = texps.indexOf('->');
    return (pos === 0) ? makeFailure(`No param types in proc texp - ${texps}`) :
...
export const parseTupleTExp2 = (texps: Sexp[]): Result<TupleTExp> => {
...
return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()) : bind(bind(splitEvenOdds(texps),
                                               (argTEs: Sexp[]) => mapResult(f, argTEs)), (TEs: NonTupleTExp[]) => makeOk(makeNonEmptyTupleTExp(TEs)));
}
/*
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseTuple2 = (tuple: TupleTExp): Result<string[]> =>
        isEmptyTupleTExp(tuple) ? makeOk(["Empty"]) :
        safe2((paramTE: string, paramTEs: string[]) => makeOk([paramTE].concat(chain(te => ['*', te], paramTEs))))
            (unparseTExp(first(tuple.TEs)), mapResult(unparseTExp, rest(tuple.TEs)));
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? safe((TEs: string []) => makeOk([...TEs]))(unparseTuple2(x)) :
...
// console.log(parseTE("(boolean * (T * T -> boolean))"));