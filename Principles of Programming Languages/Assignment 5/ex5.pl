...
% Purpose: The relation between child name and it's parent phone number,
% when the parent has a car.
%
pick_me_up(Child, Phone):-
    parents(Child, Father, _),
    parent_details(Father, Phone, true).
pick_me_up(Child, Phone):-
    parents(Child, _, Mother),
    parent_details(Mother, Phone, true).

% Signature: active_child(Name)/1
% Purpose: Children with at least 2 activities
%
active_child(Child):-
    participate(Child, Activity1),
    participate(Child, Activity2),
    Activity1 \= Activity2.

% Signature: activity_participants_list(Name, List)/2
% Purpose: The relation between activity name and a list of children
% names that participate at this activity.
%
activity_participants_list(Activity, Children):-
    activity(Activity, _),
    findall(Child, participate(Child, Activity), Children).
...
%
can_register(Child, Activity) :-
    bagof(Day, busy_days_of_child(Child, Day), List),
    activity(Activity, ActivityDay),
    not_member(ActivityDay, List).

busy_days_of_child(Child, Day) :-
    participate(Child, Activity),
