...
(define sqrt2
  (lambda (x init epsilon)
   (head (find-first (sqrt-lzl x init) (lambda (cons-lzl)(good-enough? (head cons-lzl) x epsilon))))
  )
)
...
(define get-value
  (lambda (assoc-list key)
   (if (empty? assoc-list) 'fail
       (if (equal? key (car(car assoc-list))) (cdr (car assoc-list))
...
(define get-value$
  (lambda (assoc-list key success fail)
    (if (empty? assoc-list) (fail)
       (if (equal? key (car(car assoc-list))) (success (cdr (car assoc-list)))
...
(define collect-all-values-2
 (lambda (lists key)
   (if (empty? lists) '()
       (if (not (get-value$ (car lists) key (lambda(x) x) (lambda()#f)))
