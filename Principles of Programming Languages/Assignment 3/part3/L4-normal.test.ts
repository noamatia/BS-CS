...
describe('L4 Normal Eval', () => {
    ...
    it('does not catch an error of deviding by zero', () => {
        expect(bind(parseL4(`
            (L4 (if (> 4 3) 23 (/ 5 0)))`), evalNormalProgram)).to.deep.equal(makeOk(23));
    });

    it('does not catch an error of infinity loop', () => {
        expect(bind(parseL4(`
            (L4 (define loop (lambda (x) (loop x))) (define f (lambda (x) 5)) (f (loop 0)))`), evalNormalProgram)).to.deep.equal(makeOk(5));
    });

    it('does not catch an error of null pointer', () => {
        expect(bind(parseL4(`
            (L4 (define list '(1)) (if #f (cdr list) 23))`), evalNormalProgram)).to.deep.equal(makeOk(23));
    });
});
