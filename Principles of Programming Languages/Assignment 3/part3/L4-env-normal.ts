...
import { VarDecl, CExp, ProcExp, makeProcExp } from './L4-ast';
...
// Apply-env
export const applyEnv = (env: Env, v: string): Result<Promise> =>
    isEmptyEnv(env) ? makeFailure(`var not found ${v}`) :
    isExtEnv(env) ? applyExtEnv(env, v) :
    applyRecEnv(env, v);

export const applyExtEnv = (env: ExtEnv, v: string): Result<Promise> =>
    env.vars.includes(v) ? makeOk(env.vals[env.vars.indexOf(v)]) :
    applyEnv(env.nextEnv, v);

const applyRecEnv = (env: RecEnv, v: string): Result<Promise> =>
    env.vars.includes(v) ? makeOk(makePromise(makeProcExp(env.paramss[env.vars.indexOf(v)],
