...
export const parseForExp = (params: Sexp[]): Result<ForExp> =>
    params.length !== 4 ? makeFailure("Expression not of the form (for <vardecl> <numexp> <numexp> <cexp>)") :
    parseGoodForExp(params[0], params[1], params[2], params[3]);

export const parseGoodForExp = (variable: Sexp, start: Sexp, end: Sexp, body: Sexp): Result<ForExp> =>
    ! isIdentifier(variable) ? makeFailure("First arg of for must be an identifier") :
    ! (isToken(start) && isString(start) && isNumericString(start)) ? makeFailure("Second arg of for must be a number") :
    ! (isToken(end) && isString(end) && isNumericString(end)) ? makeFailure("Third arg of for must be a number") :
    ! ((+start)<=(+end)) ? makeFailure("Invalid range") :
    bind(parseL21CExp(body),
         (value: CExp) => makeOk(makeForExp(makeVarDecl(variable), makeNumExp(+start), makeNumExp(+end), value)));
