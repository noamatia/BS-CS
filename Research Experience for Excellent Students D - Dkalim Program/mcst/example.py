...
for i_episode in range(n_rounds):
    ...
    initialize_box_map(env.box_mapping)

    for t in range(n_steps):
        env.render(render_mode, scale=scale_image)
        print(f"**********\nDEAD END : {is_dead_end(env)}\n**********")
        ...
        if hasattr(env, 'new_box_position') and env.new_box_position is not None:
            update_box_position(env.old_box_position, env.new_box_position)
