...
virus *readVirus(FILE *input){
	...
	size = fread(bufferN, 1, 2, input);

	if(size==0)
		return NULL;

	v = (virus*)malloc(sizeof(virus));
	v->SigSize = bufferN[0] + 256*bufferN[1];
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
	...
	FILE *suspected = fopen(nameOfSuspectedFile , "r+");
	char CHAR = 0x90;
...
void fix_file(){
	...
	if(fgets(str, sizeof(str) ,stdin)!=NULL){
		...
		sscanf(str, "%d%d", &signitureOffset, &signitureSize);
		kill_virus(nameOfSuspectedFile, signitureOffset, signitureSize);
	}
}
